...
function convertStep (stepJson, scenarioJson) {
    if (stepJson.description in stepNames) {
      var count = stepNames[stepJson.description]+1;
      stepNames[stepJson.description] = count;
      stepJson.description = stepJson.description+' ('+count+')';
    }
    else {
      stepNames[stepJson.description] = 1;
    }
    var stepOutput = [{
            _attr: {
    ...
    switch (stepJson.assertions[0].passed) {
        case 'true':
        ...
        case 'false':
            stepOutput.push({
                error: [
                    {
                        _attr: {
                            message: stepJson.assertions[0].errorMsg.split("\n").shift()
                        }
                    }, stepJson.assertions[0].errorMsg
                ]
            });
            break;
        case 'skipped':
